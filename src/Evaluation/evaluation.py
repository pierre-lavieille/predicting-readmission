...
def main_evaluation(clf, X_test, y_test, conf):
    ...
    dict_metrics['roc_accuracy'] = roc_auc_score(y_test, clf.predict_proba(X_test)[:,1])
    ...
    with open(conf['Outputs_path'] + conf['folder_metrics'] + 'metrics_'
              + conf['selected_dataset'] + "_" + conf['selected_model'] + '_classifier.txt', 'w') as outfile:
    ...
    plt.savefig(conf['Outputs_path'] + conf['folder_metrics'] + 'metrics_'
              + conf['selected_dataset'] + "_" + conf['selected_model'] + '_confusions_matrix.png')
    ...
    plt.legend()
    plt.savefig(conf['Outputs_path'] + conf['folder_metrics'] + 'metrics_'
              + conf['selected_dataset'] + "_" + conf['selected_model'] + '_roc_curve.png')
    return dict_metrics
